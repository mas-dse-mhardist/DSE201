...
-- Videos like by both X and any given user Y
SELECT xl.video_id
...
-- This gets a list of every video
SELECT v.video_name, sum(weighted_count) as rank
FROM (
	SELECT la.video_id, la.user_id as Y, lc*count(la.user_id) as weighted_count -- for this example, la.user_id will be 1
	FROM	(SELECT a.user_id, (SELECT log(1+count(xl.video_id))
					FROM likes xl JOIN likes yl
					ON xl.video_id = yl.video_id
					WHERE xl.user_id = 168
					AND yl.user_id = a.user_id
					AND yl.user_id != 168) as lc
		FROM likes a) w RIGHT JOIN likes la
	ON w.user_id = la.user_id
	WHERE la.user_id != 168 --don't suggest the ones that I've liked ("from every other user Y")
	GROUP BY la.video_id, la.user_id, lc
) z
JOIN video v ON z.video_id = v.video_id
